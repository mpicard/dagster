...
from dagster._core.definitions.asset_backfills import (
    AssetBackfill,
    AssetBackfillCursor,
    single_backfill_iteration,
)
from dagster._core.definitions.asset_graph import AssetGraph
from dagster._core.definitions.asset_reconciliation_sensor import bfs_asset_partition_graph
...
def test_scenario_to_completion(assets_var: str, failures: str, some_or_all: str):
    ...
    if some_or_all == "all":
        ...
        for root_asset_key in root_asset_keys:
            ...
            target_subsets_by_asset_key[
                root_asset_key
            ] = partitions_def.empty_subset().with_partition_keys(chosen_partition_keys)
            for partition_key in chosen_partition_keys:
                root_asset_partitions.update(
                    asset_graph.get_children_partitions(root_asset_key, partition_key)
                )

        def handle_node(asset_partition):
            asset_key = asset_partition.asset_key
            partitions_def = asset_graph.get_partitions_def(asset_key)
            assert partitions_def is not None
            target_subsets_by_asset_key[asset_key] = target_subsets_by_asset_key.get(
                asset_key, partitions_def.empty_subset()
            ).with_partition_keys([asset_partition.partition_key])

            return True

        bfs_asset_partition_graph(
            handle_node, initial_nodes=root_asset_partitions, asset_graph=asset_graph
        )
    else:
        assert False
    ...
    backfill = AssetBackfill(target_subsets_by_asset_key)
...
def run_backfill_to_completion(
    ...
    while not backfill.is_complete(cursor.materialized_subsets_by_asset_key):
        result1 = single_backfill_iteration(
            backfill=backfill,
        ...
        result2 = single_backfill_iteration(
            backfill=backfill,
            asset_graph=asset_graph,
            instance=instance,
